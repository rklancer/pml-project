...
```{r}
fractionNA <- sapply(rawTraining, function(col) { sum(is.na(col))/length(col) })
...
prep <- function (data) {
    data <- data[,keep]
    # cast any "integer" columns to "numeric" because read.csv
    # treats soem columns as numeric in pml-training.csv but
    # integer in pml-testing.csv
...
doPrediction <- function(training, testing) {
    ...
    # return the time, the fit object, our predictions on the testing set, and the
...
So the expected out-of-sample error is ```r round(100*(1-results$confmat$overall[["Accuracy"]]), 1)``` %.
